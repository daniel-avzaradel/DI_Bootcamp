...
function getRandom() {
    ...
    let randomOctave = Math.floor(Math.random() * 1 + 4);

    note1 = arrNotes[randomNote] + randomOctave;
    console.log(note1)

    randomNote = Math.floor(Math.random() * 12);
    randomOctave = Math.floor(Math.random() * 1 + 4);

    note2 = arrNotes[randomNote] + randomOctave;
    console.log(note2)
...
function playNote() {
    ...
    let audio = new Audio(`../sounds/${note1}.mp3`)
        audio.play();

        setTimeout(function () {
            audio = new Audio(`../sounds/${note2}.mp3`)
            audio.play();
        }, 1000)      

    let selectedNote = document.getElementById(`${note1}`)
